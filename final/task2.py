        ...
        for j in range(self.map_im.size[0]):
            for i in range(self.map_im.size[1]):
                if img_array[i, j] > up_thresh:
                    img_array[i, j] = 255   # obstacle
                else:
                    img_array[i, j] = 0     # free
...
class MapProcessor():
    ...
    def inflate_map(self, kernel, absolute=True):
        ...
        - self.inf_map_img_array: 0 = free, >0 = inflated obstacle
        """
        self.inf_map_img_array = np.zeros(self.map.image_array.shape)
        for i in range(self.map.image_array.shape[0]):
            for j in range(self.map.image_array.shape[1]):
                if self.map.image_array[i][j] == 0:   # obstacle pixel
...
class Task2(Node):
    ...
    def __init__(self, node_name='task2_node'):
        ...
        # Map generation (STATIC inflation)
        ...
        # Save a copy of the purely static inflated map
        self.static_inf_map = np.copy(self.mp.inf_map_img_array)

        # Rectangular kernel for "hard" dynamic inflation
        self.dynamic_kernel = self.mp.rect_kernel(11, 1)

        # dynamic obstacle memory (treated as "static" for this run)
        self.dynamic_obstacles = []
        self.block_threshold = 1.2   # meters
    ...
    def __scan_cbk(self, scan):
        """
        Process full scan:
        - Find LiDAR hits within block_threshold
        - If they land on static FREE space, mark them as permanent "bins"
        - Inflate + update costmap
        - If current path intersects inflated region → set replan_flag
        """
        ...
        for i, r in enumerate(ranges):
            if np.isnan(r) or np.isinf(r):
                continue
            if r > self.block_threshold:
                continue

            angle = scan.angle_min + i * scan.angle_increment

            # world coordinates of hit
            ox = r * np.cos(angle) + self.ttbot_pose.pose.position.x
            oy = r * np.sin(angle) + self.ttbot_pose.pose.position.y

            # grid coordinates
            gx, gy = self.convert_coordinates(ox, oy, grid=True)

            # bounds check
            if not (0 <= gy < self.mp.map.image_array.shape[0] and
                    0 <= gx < self.mp.map.image_array.shape[1]):
                continue

            static_value = self.mp.map.image_array[gy, gx]  # 0 = free, 255 = wall
            ...
            if (gy, gx) in self.dynamic_obstacles:
                continue

            # only keep hits in static FREE space
            if static_value > 0:
                new_obstacles.append((gy, gx))

        if len(new_obstacles) == 0:
            return

        # Register as permanent "dynamic" obstacles
        for cell in new_obstacles:
            self.dynamic_obstacles.append(cell)
            self.get_logger().warn(f"[DYNAMIC] New (permanent) obstacle at {cell}")

        # Update inflated map with these obstacles
        self.apply_soft_cost_inflation()

        # If we already have a path, see if it's now blocked
        if self.path is not None and self.check_path_collision(self.path):
            self.get_logger().warn("Path is now in collision with dynamic obstacle(s).")
    ...
    def apply_soft_cost_inflation(self):
        """
        Start from purely static inflation (self.static_inf_map),
        then mark dynamic obstacles + a rectangular neighborhood
        as fully blocked (value 1.0). This directly updates
        self.mp.inf_map_img_array, which A* will use.
        """

        self.get_logger().info("Applying dynamic inflation on top of static map")

        # start from STATIC inflated map
        grid = np.copy(self.static_inf_map)

        k = self.dynamic_kernel
        dx = k.shape[0] // 2
        dy = k.shape[1] // 2

        for (i, j) in self.dynamic_obstacles:
            for x in range(i - dx, i + dx + 1):
                for y in range(j - dy, j + dy + 1):
                    if 0 <= x < grid.shape[0] and 0 <= y < grid.shape[1]:
                        if k[x - (i - dx)][y - (j - dy)] > 0:
                            grid[x, y] = 255   # blocked

        self.mp.inf_map_img_array = grid

    def check_path_collision(self, path):
        """
        Returns True if ANY waypoint in the path lies in a cell
        that is now inflated/blocked (inf_map_img_array > 0).
        """
        grid = self.mp.inf_map_img_array
        h, w = grid.shape
            ...
            if 0 <= gy < h and 0 <= gx < w:
                if grid[gy, gx] > 0.0:
                    self.get_logger().warn(
                        f"[COLLISION] Path intersects inflated region at ({gy},{gx})"
                    )
                    return True
    ...
    def publish_obstacle_markers(self):
        """Publish RViz markers for each detected dynamic obstacle."""
        markers = MarkerArray()
        t = self.get_clock().now().to_msg()

        for idx, (gy, gx) in enumerate(self.dynamic_obstacles):
