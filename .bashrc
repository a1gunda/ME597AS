...
for i in {1..6}; do
  alias cb$i="cd \$HOME/Documents/ros2_ws && colcon build --symlink-install --packages-select task_${i} && source \$HOME/Documents/ros2_ws/install/local_setup.sh && cd"
done
...
alias task3="cbfp && ros2 launch turtlebot3_gazebo navigator.launch.py spawn_objects:=true"

gitpush() {
    if [ -z "$1" ]; then
        echo "Usage: gtpsh \"commit message\""
        return 1
    fi
    cd ~/Documents/ME597AS || return
    git add .
    git commit -m "$1"
    git push
    cd
...
source ~/Documents/sim_ws/install/local_setup.bash
source ~/Documents/ros2_ws/install/local_setup.bash
